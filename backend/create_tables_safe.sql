-- CVIA Database Schema - Safe Version
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_uploaded_files_storage_path') THEN
        CREATE INDEX idx_uploaded_files_storage_path ON uploaded_files(storage_path);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_ocr_results_file_id') THEN
        CREATE INDEX idx_ocr_results_file_id ON ocr_results(file_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resume_structured_data_file_id') THEN
        CREATE INDEX idx_resume_structured_data_file_id ON resume_structured_data(file_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resume_structured_data_ocr_result_id') THEN
        CREATE INDEX idx_resume_structured_data_ocr_result_id ON resume_structured_data(ocr_result_id);
    END IF;
END $$;

-- 5. Create updated_at trigger function (only if it doesn't exist)
...
-- 6. Create triggers for updated_at (only if they don't exist)
DO $$
BEGIN
    -- uploaded_files trigger
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_uploaded_files_updated_at') THEN
    ...
    -- ocr_results trigger
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_ocr_results_updated_at') THEN
        CREATE TRIGGER update_ocr_results_updated_at 
            BEFORE UPDATE ON ocr_results 
            FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    -- resume_structured_data trigger
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_resume_structured_data_updated_at') THEN
...
-- 7. Create a view for easy access to complete resume data
...
-- 8. Create a function to get resume statistics
...
SELECT 
    ...
    (SELECT COUNT(*) FROM uploaded_files) as uploaded_files_count,
    (SELECT COUNT(*) FROM ocr_results) as ocr_results_count,
    (SELECT COUNT(*) FROM resume_structured_data) as structured_data_count; 